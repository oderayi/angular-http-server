...
function requestListener(req, res) {
    ...
    fs.stat(safeFullFileName, function (err, stats) {
        if (!err && stats.isFile()) {
            ...
            const textCT = ['application/javascript', 'application/json', 'text/html', 'text/css'];
            ...
            if (textCT.includes(ct) && /\bdeflate\b/.test(acceptEncoding)) {
                res.writeHead(200, {
                    'Content-Encoding': 'deflate',
                    "Content-Type": ct
                });
                raw.pipe(zlib.createDeflate()).pipe(res);
            } else if (textCT.includes(ct) && /\bgzip\b/.test(acceptEncoding)) {
                res.writeHead(200, {
